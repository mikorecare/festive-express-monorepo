<template>
  <div class="packages-page min-h-screen bg-white font-sans text-slate-800">
    ...
    <section class="py-12 md:py-16 bg-white">
      <div v-if="loading" class="text-center py-20 text-slate-500">Loading packages...</div>
      <div v-else-if="!packages.length" class="text-center py-20 text-slate-500">No packages available.</div>
      ...
      >
        ...
        >
          <div class="header-left relative flex items-center gap-4">
            ...
            <h2
              v-else
              class="text-3xl font-bold text-[#0c2340]"
            >
              {{ pkg.name }}
            </h2>
          </div>
        ...
        <div class="horizontal-layout grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div class="media-column lg:col-span-7 flex flex-col bg-white rounded-xl shadow-sm border border-slate-100">
            <div class="image-wrapper relative bg-slate-100 group" @click="openLightbox(pkg)">
              ...
              >
                <img
                  ...
                  alt=""
                />
                <span v-else class="text-white text-xs font-bold">{{ pkg.name }}</span>
              </div>
            ...
            <div class="color-overlay p-5 bg-white border-t border-slate-200 text-center">
              <h4 class="text-sm font-bold text-slate-900 mb-3">Select C-9 Light Color</h4>
              <div class="flex justify-center flex-wrap gap-3">
                ...
                >
                  ...
                  <span class="text-xs font-semibold text-slate-700">
                    {{ sku.color_label || byKey(sku.color_key)?.color_label || sku.color_key }}
                  </span>
                </button>
                
              </div>
              <p v-if="!skusFor(pkg.id).length" class="text-sm text-slate-400 mt-2">
          ...
          <div class="details-column lg:col-span-5 flex flex-col gap-6">
            ...
            <div class="inclusions-block">
              ...
              <ul class="space-y-2 list-none p-0 m-0">
                ...
                >
                  ...
                  <span class="text-xs font-semibold leading-snug text-slate-700">
                    {{ row.label_override || row.inclusion_items?.name }}
                    <template v-if="row.quantity > 1"> × {{ row.quantity }}</template>
                  </span>
                </li>
              </ul>
            </div>
            ...
            <div class="p-4 bg-blue-50 border border-dashed border-blue-200 rounded-xl">
        ...
        <div class="product-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div class="product-card bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-[0_10px_25px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden">
            ...
            <div class="product-body p-6 flex flex-col flex-grow">
              ...
              <div class="options-tag bg-[#f0f4f8] text-[#0c2340] px-3 py-2 rounded-[6px] text-[0.88rem] mb-5"><strong>Color Options:</strong> Warm White, Pure White, Champagne, Candy Cane, Multicolor</div>
          ...
          <div class="product-card bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-[0_10px_25px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden">
            ...
            <div class="product-body p-6 flex flex-col flex-grow">
              ...
              <div class="options-tag bg-[#f0f4f8] text-[#0c2340] px-3 py-2 rounded-[6px] text-[0.88rem] mb-5"><strong>Color Options:</strong> Pure White</div>
...
const HOTSPOT_LAYOUT: Record<string, { top: string; left: string; match?: string }[]> = {
  joy: [
  ...
  jolly: [
    ...
    { top: '83%', left: '45%', match: 'ground' }, // or 'stake'
  ],
  merry: [
    { top: '32%', left: '72%', match: 'c9' },
    { top: '33%', left: '55%', match: 'wreath' },
...
const selectedPrice = (pkg: PackageRow) => selectedSku(pkg)?.price ?? pkg.price ?? 0
...
const scrollToSpecs = () => {
  document.getElementById('package-specs')?.scrollIntoView({ behavior: 'smooth' })
}
...
const load = async () => {
  ...
  try {
    const [pkgRes, skuRes] = await Promise.all([
      ...
      db
        .from('products')
        .select('id, name, price, stock, image_url, package_id, color_key, color_label')
        .eq('is_package', true)
        .eq('is_active', true),
    ])
...
const skusFor = (packageId: string | number) => {
  const list = skus.value.filter((s) => String(s.package_id) === String(packageId))
...
onMounted(async () => {
  await loadColors()
  await load() // your packages + skus load
