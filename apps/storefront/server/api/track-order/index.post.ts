...
export default defineEventHandler(async (event) => {
  ...
  // Transform image URLs using storage
  ...
  const transformImageUrl = (url: string | null | undefined): string | null => {
    ...
    return urlData?.publicUrl || url
  }

  const itemsWithImages = rawItems.map((item) => {
    let imageUrl = item.image_url || item.image || item.options?.image_url || imageById[String(item.product_id || item.id)] || null
  ...
  // Transform timeline (no images to transform, but keeping consistent)
