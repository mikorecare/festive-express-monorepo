<template>
  <div class="p-6 space-y-6 bg-slate-50 min-h-screen">
    ...
    >
      <div>
        ...
        <p class="text-slate-500 text-sm">
          Manage banner, steps, and footer content
        </p>
      </div>
    ...
    <div
      v-if="loading"
      class="bg-white rounded-xl border border-slate-100 p-10 text-center text-slate-500"
    >
      Loading...
    </div>

    <form role="form" v-else class="space-y-6" @submit.prevent="saveContent">
      <!-- Banner + Titles -->
      <div
      ...
      >
        ...
        >
          <div>
            ...
            <p class="text-slate-500 text-sm">
              Up to 6 steps. Add or remove as needed.
            </p>
          </div>
          <button
            type="button"
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <span
              class="text-xs font-bold uppercase tracking-wide text-brand-orange"
            >
              Step {{ index + 1 }}
            </span>
            <button
...
const emptyForm = (): HowItWorksRow => ({
  ...
  footer_description: `<p style="text-align:center">That's it. One decision. Zero hassle. All the magic.</p>
...
interface HowItWorksResponse {
  success: boolean;
  data: HowItWorksRow | null;
}

const loadContent = async () => {
  loading.value = true;
  try {
    const response = await $fetch<HowItWorksResponse>("/api/how-it-works");
