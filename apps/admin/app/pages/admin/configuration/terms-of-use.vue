...
const loadContent = async () => {
  try {
    ...
    const row = data as TermsRow | null
    if (row) {
...
const saveContent = async () => {
  ...
  try {
    if (rowId.value) {
      const { error } = await supabase.from('terms_of_use').update(payload as never).eq('id', rowId.value)
      if (error) throw error
    } else {
      const { data, error } = await supabase.from('terms_of_use').insert(payload as never).select('id').single()
      if (error) throw error
