<template>
  <div class="max-w-3xl mx-auto">
    ...
    >
      <div>
        <h1 class="text-2xl font-bold text-[#1C2D5B]">Add Package SKU</h1>
        <p class="text-slate-500 text-sm mt-1">
          Create a sellable package × color product
        </p>
      </div>
    ...
    >
      <div>
        ...
        <input
          ...
          type="text"
          class="field"
          placeholder="e.g. Joy – Warm White"
        />
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div>
          ...
          <select
            ...
            @change="
              form.color_label =
                colors.find((c) => c.color_key === form.color_key)
                  ?.color_label || ''
            "
          >
            <option value="">Select color</option>
  ...
  try {
    ...
    if (response.success) {
      ...
      if (form.value.package_id) {
        const p = packages.value.find(
          (x) => String(x.id) === form.value.package_id,
        );
...
const saveSku = async () => {
  ...
  try {
    ...
    const response = await $fetch<SaveResponse>("/api/package-skus", {
      method: "POST",
      body: payload,
    });

    if (!response.success) {
      throw new Error(response.error || "Failed to create package SKU");
    }

    showToast("Package SKU created");
    navigateTo(`/admin/products/package-skus/${response.id}`);
  } catch (e: any) {
    console.error(e);
